...
import fs from "fs";
const raw = JSON.parse(fs.readFileSync("messages.json", "utf-8"));
const result = raw.map((entry) => {
    const context = entry.before.map(msg => `${msg.author.name}: ${msg.content}`);
...
// write to JSONL
const jsonl = result.map((x) => JSON.stringify(x)).join("\n");
