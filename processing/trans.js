...
client.once("ready", async () => {
    ...
    for (const msg of json
        ...
        try {
            const message = (await rest.get(Routes.channelMessage(CHANNEL_ID, msg.ID)));
            const before = (await rest.get((Routes.channelMessages(CHANNEL_ID) +
                "?limit=4&before=" +
                msg.ID)));
            let extra = {};
            if (message.message_reference) {
                ...
                try {
                    const refMessage = (await rest.get(Routes.channelMessage(CHANNEL_ID, ref.message_id)));
                    extra = {
                        ref: {
        ...
        console.log(`Processed ${completed}/${totalMessages} — ${(completed / totalMessages * 100).toFixed(2)}% — ETA: ${etaStr}`);
    }
